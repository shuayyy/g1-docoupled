...
from legged_gym.utils.human import TargetJTMotionLib
...
def sample_int_from_float_tensor(x):
    x_floor = torch.floor(x).to(dtype=torch.long)
    is_integer = (x_floor.to(dtype=torch.float32) == x)
    rand = torch.rand_like(x)
    sampled = torch.where(rand < (x - x_floor.to(dtype=torch.float32)), x_floor, x_floor + 1)
    return torch.where(is_integer, x_floor, sampled)
class G1():
    ...
    def _super_init(self, cfg, sim_params, physics_engine, sim_device, headless):
            ...
            self.obs_buf = torch.zeros(self.num_envs, self.num_obs, device=self.device, dtype=torch.float) # Current observation for each environment.
            self.rew_buf = torch.zeros(self.num_envs, device=self.device, dtype=torch.float) # Current reward for each environment.
            self.reset_buf = torch.ones(self.num_envs, device=self.device, dtype=torch.long) # Reset flag for each environment; starts at 1 so envs reset initially.
            self.episode_length_buf = torch.zeros(self.num_envs, device=self.device, dtype=torch.long) # Number of elapsed steps in each episode.
            self.time_out_buf = torch.zeros(self.num_envs, device=self.device, dtype=torch.bool) # Whether an episode ended because of time limit.
            if self.num_privileged_obs is not None:
                self.privileged_obs_buf = torch.zeros(self.num_envs, self.num_privileged_obs, device=self.device, dtype=torch.float) # Critic-only observations for asymmetric training.
            else: 
                self.privileged_obs_buf = None # No privileged observations in this setup.
                # self.num_privileged_obs = self.num_obs
            
            self.obs_history_buf = torch.zeros(self.num_envs, self.obs_context_len, self.cfg.env.num_observations, device=self.device, dtype=torch.float) # Observation history used as temporal context.
            self.action_history_buf = torch.zeros(self.num_envs, self.action_delay + 2, self.num_actions, device=self.device, dtype=torch.float) # Recent action history, used for action delay handling.

            self.extras = {} # Extra info dictionary for logging/debug outputs.
...
#        # create envs, sim and viewer
    ...
    def _init_target_jt(self):
            # Old eager-preload path kept for reference:
            ...
            self.target_jt_lib = TargetJTMotionLib(self.cfg.human.filename, self.device, self.num_dofs)
            self.num_target_jt_seq = self.target_jt_lib.num_motions()
            print(f"Registered {self.num_target_jt_seq} target joint trajectories")
            self.target_jt_i = self.target_jt_lib.sample_motions(self.num_envs)
            ...
            self.target_jt_update_steps_int = torch.full(
                (self.num_envs,),
                sample_int_from_float(self.target_jt_update_steps),
                dtype=torch.long,
                device=self.device,
            )
            self.target_jt = None
            ...
            self.delayed_obs_target_jt_steps_int = torch.full(
                (self.num_envs,),
                sample_int_from_float(self.delayed_obs_target_jt_steps),
                dtype=torch.long,
                device=self.device,
            )
            self.target_jt_seq_len = self.target_jt_lib.get_motion_lengths(self.target_jt_i)
    ...
    def update_target_jt(self, reset_env_ids):
            ...
            update_i = (self.common_step_counter % self.target_jt_update_steps_int) == 0
            if torch.any(update_i):
                self.target_jt_j[update_i] += 1
                ...
                resample_i |= jt_eps_end_bool
                self.target_jt_update_steps_int[update_i] = sample_int_from_float_tensor(
                    torch.full((int(update_i.sum().item()),), self.target_jt_update_steps, device=self.device)
                )
                self.delayed_obs_target_jt_steps_int[update_i] = sample_int_from_float_tensor(
                    torch.full((int(update_i.sum().item()),), self.delayed_obs_target_jt_steps, device=self.device)
                )
            if self.cfg.human.resample_on_env_reset:
                self.target_jt_j[reset_env_ids] = 0
                resample_i[reset_env_ids] = True
            if torch.any(resample_i):
                new_motion_ids = self.target_jt_lib.sample_motions(int(resample_i.sum().item()))
                self.target_jt_i[resample_i] = new_motion_ids
                self.target_jt_j[resample_i] = 0
                self.target_jt_seq_len[resample_i] = self.target_jt_lib.get_motion_lengths(new_motion_ids)

            delayed_frame_ids = torch.clamp(self.target_jt_j - self.delayed_obs_target_jt_steps_int, min=0)
            self.target_jt = self.target_jt_lib.get_frames(self.target_jt_i, self.target_jt_j)
            self.delayed_obs_target_jt = self.target_jt_lib.get_frames(self.target_jt_i, delayed_frame_ids)
